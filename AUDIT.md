...
> and commit refs (numbered #1–#116). ⚠️ **#99–#112 are OPEN** — they came out of the full QA pass
...
| 117 | A phone downloaded the desktop's images, byte for byte | 🟠 High | ✅ Fixed 2026-09-16 — user: *"I hope we're not loading full size images on smaller screens."* We were. Measured across 5 routes × 2 screens: every route shipped **1.8–2.3 MB of images and the bytes were IDENTICAL** on a 360px phone and a 2560px desktop. Three things, in order of size. **(a)** `noise.png` was 773 KB — **34–43% of every page's image payload** — a 500×500 full-colour RGBA grain tile with 204 alpha levels; random colour does not compress. Regenerated greyscale at 180², quantised to 16 levels: **60.8 KB**, look unchanged (A/B'd with the jitter frozen). **(b)** The four taglines are 2048×2048 (~562 KB) and cost about **67 MB of VRAM decoded** on the devices least able to spare it; a 1024² variant is a quarter of that. **(c)** The US polaroids are ~900px rendered at 281–384 CSS px. Result on `/`: **1823 KB → 909 KB on a phone**, while a 1440 desktop at 2× correctly still gets the full-size art. ⚠️ THE TAGLINES CANNOT USE `srcset` — they are WebGL textures, there is no element for it to act on, so the choice is made at load in `txtFor()`. ⚠️ AND THE TEST IS DEVICE PIXELS, NOT CSS PIXELS: a 390px phone at 3× has 1170 real pixels and a 1440 laptop at 1× has 1440, so by CSS width alone the phone looks like the smaller screen while being the higher-resolution one. An `innerWidth < 768` test would have served the small texture to the screen that needed the big one. |
