...
| ~~98~~ | ~~On a phone the select read as a picture wiping across flat colour~~ | ~~🟠 High~~ | ✅ Fixed 2026-09-15 — user: *"when I click a card (on mobile) I can see the picture slide over it while it becomes a page. That's a bit jarring. On desktop it's much better."* Same animation, different reading: the select advances the deck a WHOLE EXTRA TURN on purpose, and on a wide frame the hero is wider than the viewport for most of that turn, so it fills the screen rather than crossing it. On a 390×760 phone the same hero is only ~390×520 — it does not cover the frame, so the turn plays out as the card leaving, the screen going flat accent (captured: one frame of nothing but brown), and the picture sweeping back in from off-frame. Portrait now takes the **shortest signed path** to front-facing instead: the card you tapped flattens and grows where it already is. ⚠️ Not zero rotation — a card tapped off to the side still turns to face you, which is the part that had to stay. Landscape is untouched; the extra turn is the homepage's signature there. |
