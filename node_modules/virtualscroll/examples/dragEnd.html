...
<head>
    ...
    <script>
        window.onload = function () {
            ...
            VirtualScroll(boxa, {
                dragStart: function (movetype, position) {
                    console.log(movetype, position);
                },
                dragging: function (orientation, position) {
                    console.log(orientation, position);
                },
                dragEnd: function (movetype, position) {
                    console.log(movetype, position);
                }
            });
        };
...
<body>
    <h3>dragEnd 鼠标拖动结束</h3><i>打开控制台查看结果</i>
    <div class="boxa">
    ...
    <div style="border-left:6px solid #ccc;padding:12px;margin:12px 0 12px 0;">
        <p>dragEnd 的 this 指向根元素 实例obj</p>
        <p>作用：拖动停止，抬起鼠标左键时单一触发</p>
        <p>movetype: 表示滚动类型，水平滚动会输出 "H"，垂直滚动会输出 "V"</p>
        <p>position：滚动条在滚动结束后，滑块的最终行程百分比 0 - 1</p>
    </div>
