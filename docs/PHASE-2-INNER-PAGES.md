...
**Next up — motion**
- [x] **E — Edge-gated scroll exits (P2)**: mid-page scroll is **free**; exits only at the page edges. Both fire past `EXIT_THRESHOLD` (800px) → `doExit(fromBottom)` → `router.push('/')`; the DOM page **unmounts on navigate**, so ONLY the WebGL scene animates (one coherent motion, no DOM-vs-scene layering). Two mirrored animations:
  - **Top edge, scroll up → `deselectChapter()`** (reverse rewind): hero snaps to its ring spot, the ring **reverse**-spins back to `preSelectRot`. Smooth from the top because the hero is already on-screen at scroll 0.
  - **Bottom edge, scroll down → `exitChapterForward()`** *(2026-06-13, `82b1296b`)* — the **mirror** of the top, built the same clean way: a single ~2.5s WebGL timeline that reassembles the ring. Mirrored bits: spins **FORWARD** (`animatedRotationY += EXIT_SPIN` 290°) — continues the entry/homepage direction so there's **no reversal** after it lands (the top's rewind *decreases* = reverse; this doesn't); and the card comes **from the bottom** — the hero scrolled off the *top*, so we mirror that offset to off-screen **below** (reposition hidden under the still-mounted opaque page) and rise it up into the ring, instead of snapping it to centre (the reported "snaps to the top" bug from reusing `deselectChapter` at the bottom). **Prod-verified on the REAL wheel path** (`/tmp/exit-fwd3.json`): `animRotY` climbs 7.069→8.537→12.130 (forward +290°, monotonic — no reversal), `carY −43→0`, DOM unmounted from the first frame, no snapped card at centre at de≈0, cards rise from below, lands on a clean homepage, zero errors. ◀ *awaiting user confirm on feel*
