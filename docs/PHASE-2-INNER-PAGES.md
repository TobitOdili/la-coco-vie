...
**Next up — motion**
- [x] **E — Edge-gated scroll exits (P2)**: mid-page scroll is **free**; exits only at the page edges. **BOTH edges use the same clean exit** *(2026-06-13, `4f11d0c6`)*: overscroll past the top **or** bottom past `EXIT_THRESHOLD` (800px) → `doExit()` → `router.push('/')` → the app.vue route watcher runs `scene.deselectChapter()` (the hero card shrinks + the ring reverse-spins back into place). The DOM page **unmounts on navigate**, so ONLY the WebGL scene animates — one coherent motion, no DOM-vs-scene layering.
- ⏸️ **E (bottom exit) — forward "drop into the deck" / literal merge — PARKED.** History + why it's parked: a scroll-coupled forward exit (`97df22da`) drove a DOM-page transform (shrink/fall/fade via `applyExit`) **over** the live WebGL scene while `setExitProgress` spun the ring → **two competing layers**, so the chapter's WebGL card was always visible spinning behind the receding page ("wine card in the bg"). Hiding the chapter's poster copies (`8fbfc3ef`) didn't fix it because the fundamental issue was the two layers. Per user direction (twice: "build back the top animation as basis"), reverted to the single-WebGL-motion exit above. **A future forward "page becomes a rotating card" variant must be built the SAME way the top exit is clean — unmount the DOM first and animate purely in WebGL** (i.e. render the page into a WebGL ring card → Phase-3 "literal merge"), NOT a DOM transform over the scene. The scene's `beginExit`/`setExitProgress`/`cancelExit`/`endExit` primitives remain in `useChapterScene.js` but are **dormant** (only the `?debug` `__exitScrub` hooks call them) — reserved for that rebuild. ⚠️ **Verification lesson:** the `__exitScrub` Browserless probe drives the SCENE in isolation and **bypasses the real wheel→DOM path** — it reported "no duplicate" while the real page still showed it. Verify the bottom exit by **driving real `wheel` events** to the bottom + overscroll (see `/tmp/exit-real.json` pattern), not by scrubbing the scene.
