...
> ⚠️ **Each film has a paired STILL** — `public/images/still-{slug}.jpg`, frame 0.04, generated by
> `npm run gen:stills`. The card's photo window IS the video texture, so without it the window is
> blank until that film plays. **Regenerate the stills whenever you replace a film**, or the window
> will show one film's first frame and then cut to a different one on hover.
...
> **✅ IN FRAMES' folders are the day in order (2026-09-07)** — `pre-wedding shoot` / `white wedding`
> / `reception` / `after-party`, in `chapterPages.js`. ⚠️ `traditional` was removed here, finishing
> the sitewide removal of the traditional marriage begun 2026-09-06; this folder was the last place
> it was still named. The grid derives its column count from the list, so adding or removing one
> stays balanced.
