...
> **Still Milla Nova PLACEHOLDERS, to swap for the couple's own:** the card **films**
> (`public/video/*-intro.mp4`), some gallery **stills**, the **favicon**, and the 4 ambient **audio**
> tracks (`public/audio/*.mp3` — Howler.js loops, one per chapter, matching the reference; keep the
> wiring, replace the files + rename off the old slugs, update `CHAPTERS[i].audio`).
